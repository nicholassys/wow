...
local whisperIcon = CreateFrame("Button", "WhisperIcon", UIParent)
whisperIcon:SetSize(25, 25)
whisperIcon:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
whisperIcon:SetFrameStrata("LOW")
whisperIcon:SetHighlightTexture("Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight")
whisperIcon:SetNormalTexture("Interface\\Icons\\Ability_Hunter_SniperShot")
whisperIcon:SetPushedTexture("Interface\\Icons\\Ability_Hunter_SniperShot")
whisperIcon:Hide()

local filteredWhispersFrame = CreateFrame("ScrollingMessageFrame", "FilteredWhispersFrame", UIParent)
filteredWhispersFrame:SetPoint("CENTER", UIParent, "CENTER", 0, 50)
filteredWhispersFrame:SetSize(500, 200)
filteredWhispersFrame:SetJustification("LEFT")
filteredWhispersFrame:SetFontObject(ChatFontNormal)
filteredWhispersFrame:SetTextColor(1, 1, 1, 1)
filteredWhispersFrame:SetMaxLines(100)
filteredWhispersFrame:SetFading(false)
filteredWhispersFrame:SetInsertMode("TOP")
filteredWhispersFrame:SetBackdrop({
        ...
        if LPaddon.checkMessage then
            for word in pairs(LPaddon.filterWords) do
                if string.find(lowercase_message, word) then
                    table.insert(LPaddon.filteredWhispers, {msg = msg, sender = sender, timestamp = time()})
                    return true
                end
            end
        end
...
if event == "CHAT_MSG_WHISPER" and #LPaddon.filteredWhispers > 0 then
    whisperIcon:Show()
end

whisperIcon:SetScript("OnClick", function(self, button, down)
    if button == "LeftButton" then
        if filteredWhispersFrame:IsShown() then
            filteredWhispersFrame:Hide()
        else
            filteredWhispersFrame:Show()
            filteredWhispersFrame:Clear()

            for _, whisper in ipairs(LPaddon.filteredWhispers) do
                filteredWhispersFrame:AddMessage(string.format("[%s] %s: %s", date("%H:%M:%S", whisper.timestamp), whisper.sender, whisper.msg))
            end
        end
    end
end)
